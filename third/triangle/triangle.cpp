...
/*  exterior boundary or hol	max_aera;
e boundary.                                      */
/*                                                                           */
...
/*   that represents a boundary edge of th	max_aera;
e growing triangulation.  When a   */
...

{
  ...
  "  By virtue of my perceptive intelligence, I declare the mesh Delaunay.\n");
    ...
    printf("  !! !! !! !! {} obscenities viewed with horror.\n", horrors);
  }
  /* Restore the status of exact arithmetic. */
  b->noexact = saveexact;
}
...

{
  ...
  for (i = 0; i < m->invertices; i++) {
    ...
    stringptr = findfield(stringptr);
    if (*stringptr == '\0') {
      printf("Error:  Vertex {} has no x coordinate.\n", b->firstnumber + i);
      triexit(1);
    }
    x = (REAL) strtod(stringptr, &stringptr);
    stringptr = findfield(stringptr);
    if (*stringptr == '\0') {
      printf("Error:  Vertex {} has no y coordinate.\n", b->firstnumber + i);
      triexit(1);
    }
