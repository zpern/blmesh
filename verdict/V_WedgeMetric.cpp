...
*/
C_FUNC_DEF VERDICT_REAL v_wedge_eqangle_skew(int num_nodes, VERDICT_REAL coordinates[][3])
{
	if (num_nodes != 6)
		return -1;
    ...
    unsigned int metrics_request_flag, WedgeMetricVals *metric_vals )
{
  ...
  {
	  metric_vals->eqangle_skew = v_wedge_eqangle_skew(num_nodes, coordinates);
  }
}
