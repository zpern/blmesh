...
C_FUNC_DEF VERDICT_REAL v_quad_maximum_angle( int /*num_nodes*/, VERDICT_REAL coordinates[][3] )
{
  ...
  {
    //max_angle = 360 - max_angle;	// need figuring out
  } 
...
/*!
  The equiangle skewness of quad
*/
C_FUNC_DEF VERDICT_REAL v_quad_eqangle_skew(int num_nodes, VERDICT_REAL coordinates[][3])
{
	/* angle of equilateral quad is 90 */
	double eq_angle = 90.0;
	double max_angle = v_quad_maximum_angle(4, coordinates);
	double min_angle = v_quad_minimum_angle(4, coordinates);
    ...
    unsigned int metrics_request_flag, QuadMetricVals *metric_vals )
{
  ...
  */
      ...
      && is_collapsed == VERDICT_FALSE )
  {
	...
	*	equiangle skewness = max[(maxangle - 90) / (180 - 90), (90 - minangle) / 90]
	*	where 90 is angle for equilateral quad
	**/
	double eq_angle = 90.0;
