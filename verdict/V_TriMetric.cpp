...
C_FUNC_DEF VERDICT_REAL v_tri_edge_ratio( int /*num_nodes*/, VERDICT_REAL coordinates[][3] )
{
	...
	for (int i=0; i<3; i++)
	{
		...
		if(side_len > max_length)
			max_length = side_len;

		if(side_len < min_length)
			min_length = side_len;
	}
	//printf("min:%f max_len:%f",min_length,max_length);
	
	return (VERDICT_REAL)max_length/min_length;
}

/*!
   the size change of a tri
*/
C_FUNC_DEF VERDICT_REAL v_tri_size_change( int /*num_nodes*/, VERDICT_REAL coordinates[][3] )
{
	return 0;
}
...
/*!
  The equiangle skewness of triangle
*/
C_FUNC_DEF VERDICT_REAL v_tri_eqangle_skew(int num_nodes, VERDICT_REAL coordinates[][3])
{
	/* angle of equilateral tri is 60 */
	double eq_angle = 60.0;
	double max_angle = v_tri_maximum_angle(3, coordinates);
	double min_angle = v_tri_minimum_angle(3, coordinates);
	double max_skew = (max_angle - eq_angle) / (180 - eq_angle);
	double min_skew = (eq_angle - min_angle) / eq_angle;

	return (VERDICT_REAL) VERDICT_MAX(max_skew, min_skew);
}
    ...
    unsigned int metrics_request_flag, TriMetricVals *metric_vals ) 
{
  ...
  */
  ...
  if( metrics_request_flag & (V_TRI_MINIMUM_ANGLE | V_TRI_MAXIMUM_ANGLE) )
  {
	...
	/** calculate the equiangle skewness of the tri, added by BHWang
	*	equiangle skewness = max[(maxangle - 60) / (180 - 60), (60 - minangle) / 60]
	*	where 60 is angle for equilateral triangle
	**/
	double eq_angle = 60.0;
	double max_skew = (metric_vals->maximum_angle - eq_angle) / (180 - eq_angle);
	double min_skew = (eq_angle - metric_vals->minimum_angle) / eq_angle;
	metric_vals->eqangle_skew = (VERDICT_REAL)VERDICT_MAX(max_skew, min_skew);
  }
  ...
  if(metrics_request_flag & V_TRI_EDGE_RATIO)
	metric_vals->edge_ratio = v_tri_edge_ratio(num_nodes, coordinates);
