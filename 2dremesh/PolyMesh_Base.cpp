...
void MPolyFace::updataNormal()
{
	...
	vec3d vector1 = v2->position() - v1->position();
	he = he->prev();
	v2 = he->fromVertex();
	vec3d vector2 = v2->position() - v1->position();
...
vec3d MPolyFace::getFaceCenter()
{
	assert(he_begin_ != nullptr);

	MHalfedge* he = he_begin_;

	vec3d getCenter(0, 0, 0);
	polynum_ = 0;
	do
	{
	...
	getCenter = vec3d(getCenter.x()/polynum_, getCenter.y() / polynum_, getCenter.z() / polynum_);
	return getCenter;
}
