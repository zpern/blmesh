...
public:
	MVert() : 
		index_(-1), point_(0, 0, 0), normal_(0, 0, 0), he_(nullptr)
	{
	}
	MVert(double x, double y, double z) : 
		index_(-1), point_(x, y, z), normal_(0, 0, 0), he_(nullptr)
	{
	}
...
public:
	...
	void setPosition(vec3d new_point) { point_ = new_point; }
	void setPosition(double x, double y, double z) { point_ = vec3d(x, y, z); }
	vec3d position() { return point_; }
	const vec3d& position() const { return point_; }
	...
	void setNormal(vec3d& new_vec) { normal_ = new_vec; }
	void setNormal(double nx, double ny, double nz) { normal_ = vec3d(nx, ny, nz); }
...
public:
	...
	vec3d tangent()
	{
		vec3d t = toVertex()->position() - fromVertex()->position();
		t.normalize();
		return t;
	}
...
public:
	...
	{
		updateVert();
		vec3d t = v1_->position() - v2_->position();
		return t.norm();
	}
