...
//=====================================================================
	...
	MVert* PolyMesh::addVertex(const vec3d& point)
	{
		MVert* w_ptr = poolV.Request();
...
// Mesh Based Method
//=====================================================================
	double PolyMesh::closestPoint(const vec3d& p, MVert* close_v)
	{
		size_t v_size = vertices_.size();
		double c_dis = 1e6;
		MVert* cv = nullptr;
		for (size_t i = 0; i < v_size; i++)
		{
			vec3d vp = vertices_[i]->position();

			double v_dis = (vp-p).norm();
	...
	MVert* PolyMesh::closestPoint(const vec3d& p)
	{
		MVert* cv = nullptr;
...
//=====================================================================
	MVert* PolyMesh::splitEdgeMakeVertex(MEdge* edge)
	{
		...
		vec3d p = v0->position()* sp + v1->position()* (1 - sp);
	...
	MEdge* PolyMesh::splitQuadrilateralTriangle(MPolyFace* face, bool isqulaty)
	{
		...
		vec3d vec0 = he0->tangent();
		vec3d vec1 = he0->tangent();
		vec3d vec2 = he0->tangent();
		vec3d vec3 = he0->tangent();

		double a0 = vec0.dot(-vec3);
		double a1 = vec1.dot(-vec0);
		double a2 = vec2.dot(-vec1);
		double a3 = vec3.dot(-vec2);
	...
	bool PolyMesh::weldVertex(MVert* v_ori, MVert* v_tar, double tolerance)
	{
		...
		if ((v_ori->position()-v_tar->position()).norm() > tolerance)
		{
		...
		for (size_t i = 0; i < v0_he_list.size(); i++)
		{
			...
			vec3d p0 = v0_he->toVertex()->position();
			for (size_t j = 0; j < v1_he_list.size(); j++)
			{
				MHalfedge* v1_he = v1_he_list[i];
				vec3d p1 = v1_he->toVertex()->position();
				if ((p0-p1).norm() <= tolerance)
				{
	...
	bool PolyMesh::weldEdge(MEdge* e0, MEdge* e1, double tolerance)
	{
		...
		vec3d h0_target = he00->tangent();
		vec3d h1_tagert = he10->tangent();

		double d_dot = h0_target.dot(h1_tagert);
		...
		if (v0 != v1_new)
		{
			if((v0->position()-v1_new->position()).norm() > tolerance)
			{
		...
		if (v1 != v0_new)
		{
			if ((v1->position()-v0_new->position()).norm() > tolerance)
			{
				Massage::Warning("The two splice edge's endpoint is two far");
				return false;
			}
...
//=====================================================================
	...
	MVert* PolyMesh::splitEdgeTriangle(MEdge* edge)
	{
		...
		MVert* vt = newVertex();
		vec3d p0 = v0->position(); vec3d p1 = v1->position();
	...
	bool PolyMesh::is_flip_ok_Triangle(MEdge* edge)
	{
		...
		MVert* vc = he0->fromVertex();
		MVert* vd = he0->toVertex();

		auto area = [](MVert* v1, MVert* v2, MVert* v3)->double {
			double detleft = (v1->x() - v3->x()) * (v2->y() - v3->y());
			double detright = (v1->y() - v3->y()) * (v2->x() - v3->x());
			double det = detleft - detright;
			return det;
		};
		double a1 = area(vc, vd, va);
		double a2 = area(vd, vc, vb);
		double a3 = area(va, vb, vd);
		double a4 = area(vb, va, vc);
		if(a1+a2>(a3+a4)*1.01|| a1 + a2 < (a3 + a4)*0.99) return false;
		double min1 = std::min(area(vc,vd,va),area(vd,vc,vb));
		double min2 = std::min(area(va,vb,vd),area(vb,va,vc));
		if (min1 > min2)
			return false;
	//	spdlog::info(min1 << " " << min2);
...
//=====================================================================
	...
	vec3d PolyMesh::calculatFaceCenter(MPolyFace* face)
	{
		if (face != nullptr) return face->getFaceCenter();
		else return vec3d(0, 0, 0);
	}

	vec3d PolyMesh::calculatEdgeCenter(MEdge* edge)
	{
