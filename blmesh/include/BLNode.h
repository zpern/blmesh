...
{
public:
	BLNode(int iNod = -1) :delete_count(0),
...
public:
	...
	void SetDistanceRatio(double val) {
	...
	void SetUpperNode(BLNode* blNode)
	{
		m_pBLUpperNode = blNode;
	}

	void SetLowerNode(BLNode* blNode)
	{
		m_pBLLowerNode = blNode;
	}
	...
	void AddUpperNode(BLNode* blNode)
	{
		m_BLUpperNode = blNode;
	}

	void AddLowerNode(BLNode* blNode)
	{
		m_BLLowerNode = blNode;
	}
#if 0
	//left-right
	bool HasLeftNode()
	...
	void AddNeigFronts(BLFront* blFront)
	{
		m_vecNeighFronts.push_back(blFront);
	}

	int NeighFrontCount()
	{
		return m_vecNeighFronts.size();
	}
	std::vector <BLFront*>& GetNeigFronts() {
		return m_vecNeighFronts;
	}
	...
	void SetBSys(bool flag, int isym = -1) { bsys = flag;  m_iSymAxis = isym; }
