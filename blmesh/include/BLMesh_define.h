...
struct   Elem
{
	Elem() :topo(BLEntityTopology::TETRAHEDRON), nconn(4) {}
...
{
	SearchTriangle() :time_stamp(std::numeric_limits<unsigned int>::max()) {};
	SearchTriangle(const std::array<int, 3>& i) :time_stamp(std::numeric_limits<unsigned int>::max()), indexs(i) {};
		...
		if (!strcmp(word, "per_face")) {
			...
			for (int j = 0; j < 3; j++) {
				...
				sscanf(line, "%lf %lf %lf\n", &cf.rotate_matrix(j, 0),
					&cf.rotate_matrix(j, 1), &cf.rotate_matrix(j, 2));
			}
			cf.reverse_matrix = cf.rotate_matrix.inverse();
			fgets(line, nbytes, fp);
			sscanf(line, "%lf %lf %lf\n", &cf.shift_vec(0),
				&cf.shift_vec(1), &cf.shift_vec(2));
		}
	}
	fclose(fp);
	return 0;
...
static bool TimeCheck()
{
	...
	if (!ret)
	{
		//spdlog::info("Unknown error: please contact the software provider of chenjj@zju.edu.cn!\n");
		//exit(0);
		return ret;
	}
	return ret;
