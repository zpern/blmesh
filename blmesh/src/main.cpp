...
void showMemoryInfo(void)
{
    ...
    // cout << "内存使用:" << pmc.WorkingSetSize / 1024/1024 << "M/" << pmc.PeakWorkingSetSize /
    // 1024 / 1024 << "M + " << pmc.PagefileUsage / 1024 / 1024 << "M/" << pmc.PeakPagefileUsage /
    // 1024 / 1024 << "M" << endl;
}
...
int main(int argc, char *argv[])
{
    ...
    auto temp_ofile = PRE::blpre(blcf, smesh, points);
    ...
    std::cout.fill(' ');
    spdlog::info("|Generate Boundary Layer Mesh Cost|{}",
                 (gen_bdymesh_end_time - gen_mesh_start_time) / CLOCKS_PER_SEC); // '|' );
    spdlog::info("|Total Check Insertion Cost       |{}",
                 blmesh->check_prism_time / CLOCKS_PER_SEC);                     // );
    spdlog::info("|Total Normal Smoothing Cost      |{}",
                 blmesh->smooth_time / CLOCKS_PER_SEC);                          //);
    std::cout.setf(ios::left, ios::adjustfield);
    std::cout.fill(' ');
    ...
    spdlog::info("|Generate Boundary Layer Mesh Cost|{}",
                 (gen_bdymesh_end_time - gen_mesh_start_time) / CLOCKS_PER_SEC);
    spdlog::info("|Total Prism Check Insertion Cost |{}",
                 blmesh->check_prism_time / CLOCKS_PER_SEC);
    spdlog::info("|Total Prism Check valid Cost     |{}",
                 (blmesh->check_prism_quality_time - blmesh->check_prism_time) / CLOCKS_PER_SEC);
    spdlog::info("|Total Normal Smoothing Cost      |{}", blmesh->smooth_time / CLOCKS_PER_SEC);
    spdlog::info("|Total Pyramid generation Cost    |{}",
                 blmesh->generate_pyramid_time / CLOCKS_PER_SEC);
    spdlog::info("|Other process In bdymesh Cost    |{}",
                 (gen_bdymesh_end_time - gen_mesh_start_time - blmesh->smooth_time -
                  blmesh->check_prism_quality_time - blmesh->generate_pyramid_time) /
                     CLOCKS_PER_SEC);
    spdlog::info("|Generate Tetrahedral Mesh Cost   |{}",
                 (gen_mesh_end_time - gen_bdymesh_end_time) / CLOCKS_PER_SEC);
    spdlog::info("|Generate Mesh Cost        (no IO)|{}",
                 (gen_mesh_end_time - gen_mesh_start_time) / CLOCKS_PER_SEC);
