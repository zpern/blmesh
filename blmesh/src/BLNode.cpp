...
const BLVector &BLNode::GetNormal() const { return m_Normal; }
...
void BLNode::GetVirtualNeigNods(BLNode **blNods, int *nBLNod, MBLNode *pnode, bool flag)
{
    ...
    for (i = 0; i < nSize; i++) {
        ...
        for (j = 0; j < nNods; j++) {
            ...
            if (blNodes[j] != this) {
                BLVector v1(pnode[blNodes[j]->GetNodIdx()].coord[0], pnode[blNodes[j]->GetNodIdx()].coord[1],
                            pnode[blNodes[j]->GetNodIdx()].coord[2]);
...
vector<BLNode *> &BLNode::GetNeigNods()
{
    ...
    for (auto *fr : m_vecNeighFronts) {
        ...
        if (seen.insert(a).second) {
            m_vecNeighNodes.push_back(a);
        }
        if (seen.insert(b).second) {
            m_vecNeighNodes.push_back(b);
        }
    }
...
double BLNode::GetBeitaVisu() { return beitaVisu; }
void BLNode::SetBeitaVisu(double val) { beitaVisu = val; }
    ...
    for (int i = 0; i < m_vecNeighFronts.size(); i++) {
        ...
        for (auto k = 0; k < numNod; k++) {
            if (node[k]->GetNodIdx() != m_iNod) {
                BLVector bv(pNodes[node[k]->GetNodIdx()].coord[0] - pNodes[m_iNod].coord[0],
                            pNodes[node[k]->GetNodIdx()].coord[1] - pNodes[m_iNod].coord[1],
                            pNodes[node[k]->GetNodIdx()].coord[2] - pNodes[m_iNod].coord[2]);
                bv.normalize();
                side.push_back(bv);
            }
        }
    ...
    for (int i = 0; i < size; i++) {
        for (int j = i + 1; j < size; j++) {
            for (int k = j + 1; k < size; k++) {
                BLVector centor = solveCenterPointOfCircle(m_vecNeighFronts[i]->GetNormal(), m_vecNeighFronts[j]->GetNormal(),
                                                           m_vecNeighFronts[k]->GetNormal());
...
double BLNode::GetHeightLength()
{
    if (expect_height_ < 0) {
        ...
        if (m_ilayNum > cf.layer_ratio) {
            ...
            ratio = pow(respect_ratio, m_ilayNum);
        }
        if (adjacent) {
