...
double BLMesh::GetAvergeLayer()
{
	return m_nPrism * 1.0 / first_layer_FrtNods;
}
	...
	for (i = 0; i < nelm; i++)
	{
		...
		if (bct == BoundaryType::per) {
			for (j = 0; j < nconn; j++)
			{
				...
				if (per_point[idx] / 10 == 0)per_point[idx] += 10;
			}
		}
#ifdef _CHECK_INTERSECTION
		//intersection 
		...
		if (bct == BoundaryType::wall || bct == BoundaryType::match ||bct==BoundaryType::adjacent)
		{
			...
			{
				int idx =m_pElems[i].conn[j];
				if (per_point[idx] % 10 == 0)per_point[idx] += 1;
				BLNode* blNodTmp = (BLNode*)m_pNodes[m_pElems[i].conn[j]].pointer;
	...
	for (i = 0; i < m_nSymBdrys; i++)
	{
		if (m_pSymBdrys[i].nconn < 0)
		{
			...
			for (j = beg; j < end; j++)
			{
				...
				{
					idx3 = (m_pElems[eidx].conn[0] + m_pElems[eidx].conn[1] + m_pElems[eidx].conn[2]) -
						   (idx1 + idx2);
			...
			// 				if(i == 15)
			// 					spdlog::info("front idx: {} ({} {} {})\n", eidx, m_pElems[eidx].conn[0], m_pElems[eidx].conn[1], m_pElems[eidx].conn[2]);
		}
	}
}
/*将一条直线加入到待选的边界中*/
void BLMesh::insertSymFace(pair<int, int> p)
{
	m_mapSymline[p.first].insert(p.second);
}
...
void BLMesh::SaveBLMeshAndFreeMemory(VM &v)
{
	...
	int sum = 0;
	int index = 0;
	m_nPrism = 0; m_nPyramid = 0; m_nTet = 0;
		...
		if (!IsElmDelete(i))
		{
			...
			if (m_pElems[i].topo == BLEntityTopology::TETRAHEDRON)
				fprintf(fout, "4 %d %d %d %d\n", m_pElems[i].conn[0],
						m_pElems[i].conn[1], m_pElems[i].conn[2], m_pElems[i].conn[3]);
		}
	}
		...
		if (!IsElmDelete(i))
		{
			...
			if (m_pElems[i].topo == EntityTopology::PYRAMID)
				fprintf(fout, "5 %d %d %d %d %d\n", m_pElems[i].conn[0],
					m_pElems[i].conn[2], m_pElems[i].conn[1],
					m_pElems[i].conn[3], m_pElems[i].conn[4]);
#else
			if (m_pElems[i].topo == BLEntityTopology::PRISM)
				fout<< "6 "<< m[m_pElems[i].conn[0]]<<" "<<
					m[m_pElems[i].conn[2]] << " " << m[m_pElems[i].conn[1]] << " " <<
					m[m_pElems[i].conn[3]] << " " << m[m_pElems[i].conn[5]] << " " << m[m_pElems[i].conn[4]]<<endl;
			else if (m_pElems[i].topo == BLEntityTopology::PYRAMID)
				fout << "5 " << m[m_pElems[i].conn[0]] << " " <<
				m[m_pElems[i].conn[2]] << " " << m[m_pElems[i].conn[1]] << " " <<
				m[m_pElems[i].conn[3]] << " " << m[m_pElems[i].conn[4]]  << endl;
			else if (m_pElems[i].topo == BLEntityTopology::TETRAHEDRON)
				fout << "4 " << m[m_pElems[i].conn[0]] << " " <<
				m[m_pElems[i].conn[1]] << " " << m[m_pElems[i].conn[2]] << " " <<
		...
		if (!IsElmDelete(i))
		{
			...
			if (m_pElems[i].topo == BLEntityTopology::PRISM)
				fprintf(fout, "6 %d %d %d %d %d %d\n", m_pElems[i].conn[0],
						m_pElems[i].conn[2], m_pElems[i].conn[1],
						m_pElems[i].conn[3], m_pElems[i].conn[5], m_pElems[i].conn[4]);
			else if (m_pElems[i].topo == BLEntityTopology::PYRAMID) {
				if (generate_pyramid) {
					fprintf(fout, "5 %d %d %d %d %d\n", m_pElems[i].conn[0],
						m_pElems[i].conn[2], m_pElems[i].conn[1],
...
int BLMesh::SaveBdry(char *filename)
{
	...
	for (i = 0; i < m_nElems; i++)
	{
		if (!IsElmDelete(i))
		{
			if (m_pElems[i].topo == BLEntityTopology::TRIANGLE)
			{
				fprintf(fout, "%d 3 %d %d %d %d\n", cnt + 1, m_pElems[i].conn[0] + 1,
						m_pElems[i].conn[1] + 1, m_pElems[i].conn[2] + 1, m_pElems[i].igom);
			...
			else if (m_pElems[i].topo == BLEntityTopology::QUADRILATERAL)
			{
				fprintf(fout, "%d 4 %d %d %d %d %d\n", cnt + 1, m_pElems[i].conn[0] + 1,
						m_pElems[i].conn[1] + 1, m_pElems[i].conn[2] + 1, m_pElems[i].conn[3] + 1, m_pElems[i].igom);
...
void BLMesh::SetSymBdryDelete(int i)
{
	m_pSymBdrys[i].nconn = -1;
}

bool BLMesh::IsSymBdryDelete(int i)
{
	return m_pSymBdrys[i].nconn < 0;
}
...
#pragma omp parallel for private(blNod, iNod, iNodNew, newpos, normal, normn) if (nFrtNods > 100000)
		...
		for (int i = 0; i < nFrtNods; i++)
		{
			...
			//regular BEM
			normn = blNodNew->GetNormal(m_pPotentialBEM, pt, m_pPt, m_nSurfNodes, m_pBElm,
										m_nSurfElems, pBc, m_pNorm, pU, m_pNodes[iNod].bsysm, iNod);
		...
		for (int i = 0; i < nFrtNods; i++)
		{
			...
			//	cout << m_bIsotropicStop << " ";
				...
				for (int j = 0; j < nblNeigFrts; j++)
				{
					...
					if (max(0.1 * ave_front_size + 0.9 * blNeigFrts[j]->GetFrontSize(), 0.30 * ave_front_size + 0.7 * blNeigFrts[j]->GetFrontSize()) > 0.95 * blFrtNods[j]->GetHeightLength()/ m_bIsotropicStop)
					{
						iso_stop = false;
					}
...
#define POWER 1
	...
	for (int i = 0; i < nNeigNods; i++)
	{
		...
		int id = bnod->GetNodIdx();
#ifdef POWER
		double length = 1.0 / BLVector(pNodes[id].coord[0] - pNodes[blNod->GetNodIdx()].coord[0], pNodes[id].coord[1] - pNodes[blNod->GetNodIdx()].coord[1], pNodes[id].coord[2] - pNodes[blNod->GetNodIdx()].coord[2]).magnitude();
...
#ifdef AVERC
	if (nNeigNods > 0)
		averC = averC / nNeigNods - BLVector(pNodes[blNod->GetNodIdx()].coord[0], pNodes[blNod->GetNodIdx()].coord[1], pNodes[blNod->GetNodIdx()].coord[2]);
	averC.normalize();
#endif
#ifdef POWER
	if (lengthSum > 0)
...
void BLMesh::SmoothHorNodeNorm(BLNode *blNod, MBLNode *pNodes)
{
	...
	if (lowerNormal * newnormal < acos(angle / 180 * PI))
	{ //acos(3.6/180*PI)
...
double BLMesh::GetDistance(BLNode *nod)
{
	...
	{
		BLVector neop(m_pNodes[front_arr[i]->GetNodIdx()].coord[0], m_pNodes[front_arr[i]->GetNodIdx()].coord[1], m_pNodes[front_arr[i]->GetNodIdx()].coord[2]);
...
void BLMesh::PreCheckPrismValid(BLFront *blFront)
{
	...
	BLVector e1 = BLVector(m_pNodes[conn[5]].coord[0] - m_pNodes[conn[3]].coord[0], m_pNodes[conn[5]].coord[1] - m_pNodes[conn[3]].coord[1], m_pNodes[conn[5]].coord[2] - m_pNodes[conn[3]].coord[2]).normalized();
...
void BLMesh::prePropagate(BLFront *blFront)
{
	...
	else
	{
		...
		BLVector up_front_normal = -1 * BLVector(m_pNodes[conn[5]].coord[0] - m_pNodes[conn[3]].coord[0], m_pNodes[conn[5]].coord[1] - m_pNodes[conn[3]].coord[1], m_pNodes[conn[5]].coord[2] - m_pNodes[conn[3]].coord[2]) ^ BLVector(m_pNodes[conn[4]].coord[0] - m_pNodes[conn[3]].coord[0], m_pNodes[conn[4]].coord[1] - m_pNodes[conn[3]].coord[1], m_pNodes[conn[4]].coord[2] - m_pNodes[conn[3]].coord[2]);
...
void BLMesh::test()
{
	double A[3] = {1.266530, -0.256233, -0.040692},
		   B[3] = {1.339154, -0.255740, -0.016062},
		   C[3] = {1.280364, -0.253356, 0.051173},
		   O[3] = {1.289233, -0.255227, 0.030649},
		   P[3] = {1.303460, -0.272334, 0.038875},
		   Q[3] = {1.295262, -0.254155, 0.024839};
				...
				for (i = 0; i < 3; i++)
				{
					if (!used[i])
					{
						...
						if (bcrepramid)
						{
							...
							thick /= pow(cf.ratio2, ptr->GetLayerNum() - layer);
							if (layer < cf.layer_ratio)
							{
...
bool BLMesh::CheckPrismVolumn(int nconn,int *conn)
{
	...
	double volumn1 = (gprism[3] - gprism[0]) * ((gprism[1] - gprism[0]) ^ (gprism[2] - gprism[0])) / 6 + (gprism[3] - gprism[1]) * ((gprism[4] - gprism[1]) ^ (gprism[5] - gprism[1])) / 6 + (gprism[3] - gprism[1]) * ((gprism[5] - gprism[1]) ^ (gprism[2] - gprism[1])) / 6;

	double volumn2 = (gprism[4] - gprism[1]) * ((gprism[2] - gprism[1]) ^ (gprism[0] - gprism[1])) / 6 + (gprism[4] - gprism[2]) * ((gprism[5] - gprism[2]) ^ (gprism[3] - gprism[2])) / 6 + (gprism[4] - gprism[2]) * ((gprism[3] - gprism[2]) ^ (gprism[0] - gprism[2])) / 6;
	//cout << volumn << endl;
...
bool BLMesh::CheckPrismValidity(int nconn, int *conn, int *pidx)
{
	...
	double fprismqual = FLT_MAX, ftmp, Jackbin, mjkb1, mjkb2, mjkb3;
	BLVector gprism[6];
	constexpr int eps_eta_kxi[6][3] = {{0, 0, 0}, {1, 0, 0}, {0, 1, 0}, {0, 0, 1}, {1, 0, 1}, {0, 1, 1}}; //六个顶点
	...
	for (int i = 0; i < 6; ++i)
	{
		...
		Jackbin = jackb1.x * jackb2.y * jackb3.z +
				  jackb2.x * jackb3.y * jackb1.z +
				  jackb3.x * jackb1.y * jackb2.z -
				  jackb2.x * jackb1.y * jackb3.z -
				  jackb1.x * jackb3.y * jackb2.z -
				  jackb3.x * jackb2.y * jackb1.z;
...
bool BLMesh::CheckIntersection(BLFront *blFront, bool *used_by_neigh_front, int *tridx)
{
	...
	//			continue;
	//		if (!is_inserect&&Octree::tri_overlap_test(itri[k], itri[j], m_ocTree->pOctreeAgent->getNod(), m_ocTree->pOctreeAgent->getElm())) {
	//			is_inserect = true;
	//			break;
...
void BLMesh::FixHightRatio(BLNode *blNod, MBLNode *pNodes)
{
	...
	{
		length += ((BLVector(pNodes[i->GetNodIdx()].coord[0], pNodes[i->GetNodIdx()].coord[1], pNodes[i->GetNodIdx()].coord[2])) - COORD).magnitude();
	}
...
void BLMesh::RemoveBox(int npt, int &nbdry, double *pt, int *elm)
{
	MeshOrient::removeBox(npt, pt, nbdry, elm);
}
	...
	for (int i = 0; i < m_nElems; i++)
	{
		if (!IsElmDelete(i))
		{
			if (m_pElems[i].topo == BLEntityTopology::TRIANGLE)
			{
				...
				if (m_pElems[i].conn[0] == m_pElems[i].conn[1] || m_pElems[i].conn[0] == m_pElems[i].conn[2] || m_pElems[i].conn[1] == m_pElems[i].conn[2])
				{
					SetElmDelete(i);
					deTriangle++;
					m_nTri--;
	...
	for (int i = 0; i < m_nElems; i++)
	{
		if (!IsElmDelete(i))
		{
			if (m_pElems[i].topo == BLEntityTopology::PRISM)
			{
				...
				else if (m_pElems[i].conn[0] == m_pElems[i].conn[1] || m_pElems[i].conn[0] == m_pElems[i].conn[2] || m_pElems[i].conn[1] == m_pElems[i].conn[2])
				{
					SetElmDelete(i);
					BLVector co;
					for (int j = 0; j < 6; j++)
...
void BLMesh::NarrowPointperturbation() {
...
void BLMesh::GenOuterMesh(int smooth_attempt)
{
	...
	try
	{
		API_Tetrahedralize_Single_Domain(
			nbpt, bpt, nbelm, belm,
...
void BLMesh::UpdateDomainGrid(int ngp, int nbp, int nel, double *g_coordx, double *g_coordy, double *g_coordz, int *gtopu, int **l_to_g)
{
	...
	for (int i = 0; i < m_nElems; i++)
	{
		if (m_pElems[i].nconn == 4)
			if (GEOM_FUNC::orient3d(m_pNodes[m_pElems[i].conn[0]].coord, m_pNodes[m_pElems[i].conn[1]].coord, m_pNodes[m_pElems[i].conn[2]].coord, m_pNodes[m_pElems[i].conn[3]].coord) > 0)
			{
				//	cout<<GEOM_FUNC::orient3d(m_pNodes[m_pElems[i].conn[0]].coord, m_pNodes[m_pElems[i].conn[1]].coord, m_pNodes[m_pElems[i].conn[2]].coord, m_pNodes[m_pElems[i].conn[3]].coord);
				//	cout << " " <<  i << endl;
				//throw(std::exception());
			}
	}
...
void BLMesh::RemoveNonManifoldFront()
{
	...
	while (it != m_vBdyFront.end())
	{
		...
		if (j != num_nei_front)
		{ //周围的front有上层front，意味着这个三角形的某个边将会长成四边形，不需要填补
			it++;
			continue;
		}
		if (num_neigh_idx.empty())
		{ //无上层节点
			it++;
			continue;
		}
		...
		while (tmp->GetUpperNode())
		{
			...
			if (edge_hash[pair<int, int>(std::min(pointl, pointr), std::max(pointl, pointr))] > 2 || (edge_hash[pair<int, int>(std::min(pointl, pointr), std::max(pointl, pointr))] == 2 && m_pNodes[pointl].bsysm))
		...
		while (blNodCur->GetUpperNode())
		{
			...
			if (need_to_generate)
			{
				...
				if (ret)
				{ //the new created triangle intersect with existing mesh elements
...
void BLMesh::RemoveOutbdry(int nRmBdry, int *pRmBdry)
{
	...
	if (nRmBdry + cnt != noutbdry)
		cout << endl
			 << "should remove" << nRmBdry << " but remove" << delete_count << " " << noutbdry - cnt << " front" << endl;
	noutbdry = cnt;
		...
		for (int i = 0; i < 3; i++)
		{
			...
			if (std::find(m_pNodes[idx1].isymfc.begin(), m_pNodes[idx1].isymfc.end(), ifc) != m_pNodes[idx1].isymfc.end() ||
				...
				else
				{
					...
					{ // I think it should not come to here too.Actually, it comes to here
						conn[0] = pid1;
						conn[1] = pid2;
						AddSymBdry(2, conn, BLEntityTopology::LINE);
						insert_sym(conn[0], conn[1]);
...
void BLMesh::SetSizingFunction(std::function<double(std::array<double, 3>)> func)
{
	sizefuntion = func;
}

std::function<double(std::array<double, 3>)> &BLMesh::GetSizingFunction()
{
	return sizefuntion;
}
