...
void main(int argc, char* argv[]){
	...
	//cout << "cf.layer_nu" << cf.layer_num << endl;
	...
	if (use_per) {
		for (auto i : blconfig.per) {
			cf.perfc.push_back(i);
		}
		for (int i = 0; i < 9; i++)
...
// 
	...
	//blmesh->CheckElemNum(itmck);
	//blmesh->RemvNodElm();
	//blmesh->SaveBLMesh(blmfile);
#ifdef _DEBUG
	...
	if (genoutmesh) {// If there is no pyramid, then there is no prism
	...
	std::cout.fill(' ');
	//spdlog::info(std::setiosflags(ios::left) << std::setfill(' ') << endl);
	...
	spdlog::info("|Total Ccost        (including IO)|{}",  (end_time - start_time) / CLOCKS_PER_SEC );
	spdlog::info("+================================================+" );
	

	return ret;
#endif
	//blmesh->SaveBdry(bdryfileo);
}
