			...
			int *npt, double **gpt, int *nelm, int** elm)
{
...
#endif
	generator.calcDens();
	spdlog::info("Initialization time: {}s.\n", ((double)(clock()-prevClock))/CLOCKS_PER_SEC);
	prevClock = clock();

	generator.bndPntInst();
	spdlog::info("Boundary insertion time: {}s.\n", ((double)(clock()-prevClock))/CLOCKS_PER_SEC);
	prevClock = clock();

	generator.recoverBnds();
	generator.writevtk(vtkfile);
	generator.clrOuterEles();
	spdlog::info("Boundary recover time: {}s.\n", ((double)(clock()-prevClock))/CLOCKS_PER_SEC);
	prevClock = clock();

	while (generator.innerPntInst()) ;
	spdlog::info("Field point insertion time: {}s.\n", ((double)(clock()-prevClock))/CLOCKS_PER_SEC);
	prevClock = clock();

	generator.smooth();
	spdlog::info("Smooth time: {}s.\n", ((double)(clock()-prevClock))/CLOCKS_PER_SEC);
	prevClock = clock();
	...
	//generator.writevtk(vtkfile);
	generator.output();
	spdlog::info("Create Points:{}. Accepted Points:{}.\n", ISO2D::g_nCreatePnts, ISO2D::g_nAcceptPnts);

	spdlog::info("Postprocessing (including output) time: {}s.\n", ((double)(clock()-prevClock))/CLOCKS_PER_SEC);
	prevClock = clock();

	spdlog::info("Total time: {}s.\n", ((double)(clock()-startGenTime))/CLOCKS_PER_SEC);
