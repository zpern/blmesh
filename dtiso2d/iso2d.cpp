...
bool DTIso2D::readFr2(const char* fname)
{
	...
	if (fname)
	{
		...
		{
			spdlog::info("Error: Not enough memory for input boundaries!\n");
			fclose(fp);
			return false;
		}
		if (m_nBnds + INIT_NOD_NUM > INIT_ALLOC_NOD_NUM)
		{
			spdlog::info("Error: Not enough memory for input nodes!\n");
			fclose(fp);
			return false;
		}
		for (i = 0; i < m_nBnds; i++)
...
bool DTIso2D::readBa2(const char* fname)
{
	...
	if (fname)
	{
		...
		if (!fp)
		{
			spdlog::info("Warning: cannot read file %s\n", fname);
			return false;
		}
		fgets(cTok,100,fp);
...
#ifdef _DEBUG
	spdlog::info("{} of totally {} nodes have unmatched spacing value.\n", ctUnmatch, m_nNodes);
#endif
...
int DTIso2D::findFirstEle(POINT pnt, INTEGER *ele)
{
	...
	//				addTstEle(iSrch);//?
				...
				/*	for (i = 0; i <DIM; i++)
					...
					if (i >= DIM)
					{
						spdlog::info("cannot find first triangle!\n");
						//go to error handle
					}*/
	#endif //_ERROR_CHK
					continue;
...
//					addTstEle(iSrch);
					...
					if (cri < -1e-15)
					{/*incircle cirteria is broken*/
						spdlog::info("volume is near zero!!\n");
						addDeleted(iSrch);
						addTreeSearch(iSrch);
					}		
...
//					if (j > DIM)
//					{
//						spdlog::info("can not find uncommon point!!\n");
//						//go to error handle
//					}
//					
//#endif //_ERROR_CHK				
...
//#ifdef _ERROR_CHK
	...
	{
		spdlog::info("the number of faces & the number of nodes is imbalant!!");
		nCase = 3;
		goto RECOVERED;
		//go to error handle
	}
...
bool DTIso2D::updateNeigInfo()
{
	...
	for (i = 0; i < m_vecAddEles.size(); i++)
	{
		...
		if (iNeig >= 0) //a valid neighbour
		{
			...
			{
				bChk = false;
				spdlog::info("cannot set neighboring info. of elements near the cavity correctly!\n");
				break;
			}
#endif //_ERROR_CHK
		}
...
int DTIso2D::writePl2(char *fname)
{
	...
	if (!fp)
	{
		spdlog::info("Cannot open file %s.", fname);
		return 2;
	}
