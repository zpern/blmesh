...
TEST(API_Gen_Tetra_Mesh, intersection) {
	...
	std::string filename = "TetraMeshing.txt";
	std::ifstream fin(filename);
	std::string trash;
	fin >> trash >> expan_beta;
	fin >> nSF >> nSN;
	...
	for (int i = 0; i < nSF; i++) {
		for (int j = 0; j < 3; j++) {
			maxNodeId = std::max(maxNodeId, pnSFFm[3 * i + j]);
		}
	}


	// error node id
	ASSERT_GT(nSN+1,maxNodeId);
	int nopt = 0;
	try {
		API_Gen_Tetra_Mesh(
			/*  ------------------------- 输入参数 -------------------------- */
