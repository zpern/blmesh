...
double size_f(std::array<double, 3> point) {/*自定义函数示例 */
	return API_Sizing_Query(point.data());
}
TEST(API_Gen_Tetra_Mesh, cubic_desity) {
	...
	pfun = size_f; /*尺寸函数指针，用户也可以自定义函数，如上面的test函数 */
	int pnMN_d, pnME_d;
	ASSERT_NO_THROW(
		API_Gen_Tetra_Mesh(
			/* ------------------------- 输入参数 -------------------------- */
	...
	ASSERT_GT(pnME, nSF);
	ASSERT_GT(pnMN, nSN);
	ASSERT_GT(pnME_d, pnME*1.05);
	ASSERT_GT(pnMN_d, pnMN*1.05);
