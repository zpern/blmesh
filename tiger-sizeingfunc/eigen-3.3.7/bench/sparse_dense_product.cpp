...
int main(int argc, char *argv[])
{
  ...
  for (float density = DENSITY; density>=MINDENSITY; density*=0.5)
  {
    ...
    #ifdef DENSEMATRIX
    {
      spdlog::info("Eigen Dense\t" << density*100 << "%\n");
      ...
      for (int k=0; k<REPEAT; ++k)
        v2 = m1 * v1;
      timer.stop();
      spdlog::info("   a * v:\t" << timer.best() << "  " << double(REPEAT)/timer.best() << " * / sec " << endl);
      ...
      for (int k=0; k<REPEAT; ++k)
        v2 = m1.transpose() * v1;
      timer.stop();
      spdlog::info("   a' * v:\t" << timer.best() << endl);
    }
    #endif

    // eigen sparse matrices
    {
      spdlog::info("Eigen sparse\t" << sm1.nonZeros()/float(sm1.rows()*sm1.cols())*100 << "%\n");

      BENCH(asm("#myc"); v2 = sm1 * v1; asm("#myd");)
      spdlog::info("   a * v:\t" << timer.best()/REPEAT << "  " << double(REPEAT)/timer.best(REAL_TIMER) << " * / sec " << endl);


      BENCH( { asm("#mya"); v2 = sm1.transpose() * v1; asm("#myb"); })

      spdlog::info("   a' * v:\t" << timer.best()/REPEAT << endl);
    }

//     {
//       DynamicSparseMatrix<Scalar> m1(sm1);
//       spdlog::info("Eigen dyn-sparse\t" << m1.nonZeros()/float(m1.rows()*m1.cols())*100 << "%\n");
//
//       BENCH(for (int k=0; k<REPEAT; ++k) v2 = m1 * v1;)
//       spdlog::info("   a * v:\t" << timer.value() << endl);
//
...
//     }
    ...
    #ifndef NOGMM
    {
      spdlog::info("GMM++ sparse\t" << density*100 << "%\n");
    ...
    #ifndef NOUBLAS
    {
      spdlog::info("ublas sparse\t" << density*100 << "%\n");
