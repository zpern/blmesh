...
{
  spdlog::info(name << "..." << std::flush);
  BenchTimer timer;
  timer.start();
  SparseLLT<EigenSparseSelfAdjointMatrix,Backend> chol(sm1, flags);
  timer.stop();
  spdlog::info(":\t" << timer.value() << endl);

  spdlog::info("  nnz: " << sm1.nonZeros() << " => " << chol.matrixL().nonZeros() );
//   spdlog::info("sparse\n" << chol.matrixL() << "%\n");
}
...
//   float density = 0.5;
  {
    ...
    spdlog::info("Generate sparse matrix (might take a while)...\n");
...
//       spdlog::info("dense:\t" << timer.value() << endl);
...
//       spdlog::info("taucs:\t" << timer.value() << endl);
...
//       c.method[0] = 1;
      ...
      timer.stop();
      spdlog::info("cholmod/analyze:\t" << timer.value() << endl);
      timer.reset();
      timer.start();
      cholmod_factorize(&A, L, &c);
      timer.stop();
      spdlog::info("cholmod/factorize:\t" << timer.value() << endl);
