...
int main(int argc, char *argv[])
{
  ...
  if(need_help)
  {
    spdlog::info(argv[0] << " r<nb rows> c<nb columns> n<non zeros per column> t<nb tries> p<nb repeats>\n");
    return 1;
  }

  spdlog::info("SpMV " << rows << " x " << cols << " with " << nnzPerCol << " non zeros per column. (" << repeats << " repeats, and " << tries << " tries)\n\n");
  ...
  while (nnzPerCol>=4)
  {
    ...
    #ifdef DENSEMATRIX
    {
      ...
      spdlog::info("Dense       " << t.value()/repeats << "\t");

      SPMV_BENCH(res = dm.transpose() * sm);
      spdlog::info(t.value()/repeats << endl);
    }
    #endif
    ...
    // eigen sparse matrices
    {
      ...
      spdlog::info("Eigen       " << t.value()/repeats << "\t");
...
//       BENCH();
//       timer.stop();
//       spdlog::info("   a * b:\t" << timer.value() << endl);
...
//       spdlog::info("   a * b:\t" << timer.value() << endl);
    ...
    #ifdef OSKI
    {
      ...
      spdlog::info("OSKI        " << t.value()/repeats << "\t");

      SPMV_BENCH( oski_MatMult(om, OP_TRANS, 1, ov, 0, ores) );
      spdlog::info(t.value()/repeats );
      ...
      SPMV_BENCH( oski_MatMult(om, OP_NORMAL, 1, ov, 0, ores) );
      spdlog::info("OSKI tuned  " << t.value()/repeats << "\t");

      SPMV_BENCH( oski_MatMult(om, OP_TRANS, 1, ov, 0, ores) );
      spdlog::info(t.value()/repeats << "\t(" << tuning <<  ")\n");
    ...
    #ifndef NOUBLAS
    {
      ...
      spdlog::info("ublas       " << t.value()/repeats << "\t");
    ...
    #ifndef NOGMM
    {
      ...
      spdlog::info("GMM++       " << t.value()/repeats << "\t");
    ...
    #ifndef NOMTL
    {
      ...
      spdlog::info("MTL4        " << t.value()/repeats << "\t");
