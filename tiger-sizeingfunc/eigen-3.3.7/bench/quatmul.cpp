...
template<typename Quat> void bench(const std::string& label)
{
  ...
  spdlog::info(label << " default " << 1e3*t.best(CPU_TIMER) << "ms  \t" << 1e-6*double(rep)/(t.best(CPU_TIMER)) << " M mul/s\n");
  
  BENCH(t, tries, rep, quatmul_novec(a,b,c));
  spdlog::info(label << " novec   " << 1e3*t.best(CPU_TIMER) << "ms  \t" << 1e-6*double(rep)/(t.best(CPU_TIMER)) << " M mul/s\n");
}
