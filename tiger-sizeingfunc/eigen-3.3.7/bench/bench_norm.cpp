...
#define BENCH_PERF(NRM) { \
  ...
  spdlog::info(#NRM << "\t" << tf.value() << "   " << td.value() <<  "    " << tcf.value() ); \
}
...
void check_accuracy(double basef, double based, int s)
{
  ...
  spdlog::info("reference\t" << std::sqrt(double(s))*yf << "\t" << std::sqrt(double(s))*yd );
  spdlog::info("sqsumNorm\t" << sqsumNorm(vf) << "\t" << sqsumNorm(vd) );
  spdlog::info("hypotNorm\t" << hypotNorm(vf) << "\t" << hypotNorm(vd) );
  spdlog::info("blueNorm\t" << blueNorm(vf) << "\t" << blueNorm(vd) );
  spdlog::info("pblueNorm\t" << pblueNorm(vf) << "\t" << pblueNorm(vd) );
  spdlog::info("lapackNorm\t" << lapackNorm(vf) << "\t" << lapackNorm(vd) );
  spdlog::info("twopassNorm\t" << twopassNorm(vf) << "\t" << twopassNorm(vd) );
  spdlog::info("bl2passNorm\t" << bl2passNorm(vf) << "\t" << bl2passNorm(vd) );
}
...
void check_accuracy_var(int ef0, int ef1, int ed0, int ed1, int s)
{
  ...
  //spdlog::info("reference\t" << internal::sqrt(double(s))*yf << "\t" << internal::sqrt(double(s))*yd );
  spdlog::info("sqsumNorm\t"  << sqsumNorm(vf)  << "\t" << sqsumNorm(vd)  << "\t" << sqsumNorm(vf.cast<long double>()) << "\t" << sqsumNorm(vd.cast<long double>()) );
  spdlog::info("hypotNorm\t"  << hypotNorm(vf)  << "\t" << hypotNorm(vd)  << "\t" << hypotNorm(vf.cast<long double>()) << "\t" << hypotNorm(vd.cast<long double>()) );
  spdlog::info("blueNorm\t"   << blueNorm(vf)   << "\t" << blueNorm(vd)   << "\t" << blueNorm(vf.cast<long double>()) << "\t" << blueNorm(vd.cast<long double>()) );
  spdlog::info("pblueNorm\t"  << pblueNorm(vf)  << "\t" << pblueNorm(vd)  << "\t" << blueNorm(vf.cast<long double>()) << "\t" << blueNorm(vd.cast<long double>()) );
  spdlog::info("lapackNorm\t" << lapackNorm(vf) << "\t" << lapackNorm(vd) << "\t" << lapackNorm(vf.cast<long double>()) << "\t" << lapackNorm(vd.cast<long double>()) );
  spdlog::info("twopassNorm\t" << twopassNorm(vf) << "\t" << twopassNorm(vd) << "\t" << twopassNorm(vf.cast<long double>()) << "\t" << twopassNorm(vd.cast<long double>()) );
//   spdlog::info("bl2passNorm\t" << bl2passNorm(vf) << "\t" << bl2passNorm(vd) << "\t" << bl2passNorm(vf.cast<long double>()) << "\t" << bl2passNorm(vd.cast<long double>()) );
}
