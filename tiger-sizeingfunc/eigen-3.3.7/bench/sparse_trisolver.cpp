...
int main(int argc, char *argv[])
{
  ...
  for (float density = DENSITY; density>=MINDENSITY; density*=0.5)
  {
    ...
    if (!densedone)
    {
      ...
      spdlog::info("   colmajor^-1 * b:\t" << timer.value() << endl);
//       std::cerr << x.transpose() << "\n";

      BENCH(x = m2.marked<UpperTriangular>().solveTriangular(b);)
      spdlog::info("   rowmajor^-1 * b:\t" << timer.value() << endl);
//       std::cerr << x.transpose() << "\n";
    }
    #endif
    ...
    {
      spdlog::info("Eigen sparse\t" << density*100 << "%\n");
...
//       spdlog::info("   colmajor^-1 * b:\t" << timer.value() << " (inplace)" << endl);
//       std::cerr << x.transpose() << "\n";
//
//       x = b;
//       BENCH(sm2.inverseProductInPlace(x);)
//       spdlog::info("   rowmajor^-1 * b:\t" << timer.value() << " (inplace)" << endl);
//       std::cerr << x.transpose() << "\n";
    }
    ...
    #ifdef CSPARSE
    {
      spdlog::info("CSparse \t" << density*100 << "%\n");
      cs *m1;
      eiToCSparse(sm1, m1);
...
//       std::cerr << x << "\n";
    ...
    // bench small matrices (in-place versus return bye value)
    {
      ...
      spdlog::info("4x4 :\t" << timer.value() << endl);
    }

    {
      ...
      spdlog::info("4x4 IP :\t" << timer.value() << endl);
    }
  #endif
