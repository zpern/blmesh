...
int main(int argc, char *argv[])
{
  ...
  for (float density = DENSITY; density>=MINDENSITY; density*=0.5)
  {
    ...
    spdlog::info("Non zeros: " << sm1.nonZeros()/float(sm1.rows()*sm1.cols())*100 << "%\n");

    // eigen sparse matrices
    {
      BENCH(for (int k=0; k<REPEAT; ++k) sm3 = sm1.transpose();)
      spdlog::info("  Eigen:\t" << timer.value() << endl);
    }
    ...
    #ifdef CSPARSE
    {
      ...
      spdlog::info("  CSparse:\t" << timer.value() << endl);
    }
    #endif
    ...
    #ifndef NOGMM
    {
      ...
      spdlog::info("  GMM:\t\t" << timer.value() << endl);
    }
    #endif
    ...
    #ifndef NOMTL
    {
      ...
      spdlog::info("  MTL4:\t\t" << timer.value() << endl);
    }
    #endif
