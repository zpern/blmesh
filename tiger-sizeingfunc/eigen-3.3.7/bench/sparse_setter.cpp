...
//         stencil.coeffRef(ij.x(), ij.y()) = 1;
  ...
  CHECK_MEM
    ...
    #ifdef DENSEMATRIX
    {
      ...
      spdlog::info("Eigen Dense\t" << timer.value() );
    }
    #endif
...
//       spdlog::info("Eigen fillrand\t" << timer.value() );
//     }
    {
      BENCH(setrand_eigen_dynamic(coords,values);)
      spdlog::info("Eigen dynamic\t" << timer.value() );
    }
//     {
//       BENCH(setrand_eigen_compact(coords,values);)
//       spdlog::info("Eigen compact\t" << timer.value() );
//     }
    {
      BENCH(setrand_eigen_sumeq(coords,values);)
      spdlog::info("Eigen sumeq\t" << timer.value() );
    }
    {
//       BENCH(setrand_eigen_gnu_hash(coords,values);)
//       spdlog::info("Eigen std::map\t" << timer.value() );
    }
    {
      BENCH(setrand_scipy(coords,values);)
      spdlog::info("scipy\t" << timer.value() );
    }
    #ifndef NOGOOGLE
    {
      BENCH(setrand_eigen_google_dense(coords,values);)
      spdlog::info("Eigen google dense\t" << timer.value() );
    }
    {
      BENCH(setrand_eigen_google_sparse(coords,values);)
      spdlog::info("Eigen google sparse\t" << timer.value() );
    }
    #endif
...
//       spdlog::info("ublas mapped\t" << timer.value() );
    }
    {
      BENCH(setrand_ublas_genvec(coords,values);)
      spdlog::info("ublas vecofvec\t" << timer.value() );
    }
    /*{
      timer.reset();
      timer.start();
      ...
      timer.stop();
      spdlog::info("ublas comp\t" << timer.value() );
    }
    {
      timer.reset();
      timer.start();
      ...
      timer.stop();
      spdlog::info("ublas coord\t" << timer.value() );
    }*/
    #endif
