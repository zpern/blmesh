...
void doEigen(const char* name, const EigenSparseMatrix& sm1, const VectorX& b, VectorX& x, int flags = 0)
{
  ...
  timer.stop();
  if (ok)
    spdlog::info("  solve:\t" << timer.value() << endl);
  else
    spdlog::info("  solve:\t" << " FAILED" << endl);

  //spdlog::info(x.transpose() );
}
...
//   float density = 0.5;
  {
    ...
    if (!densedone)
    {
      ...
      timer.stop();
      spdlog::info("Eigen/dense:\t" << timer.value() << endl);
