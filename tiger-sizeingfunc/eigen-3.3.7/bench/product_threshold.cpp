...
{
  if(mode==InnerProduct) spdlog::info("i");
  if(mode==OuterProduct) spdlog::info("o");
  if(mode==CoeffBasedProductMode) spdlog::info("c");
  if(mode==LazyCoeffBasedProductMode) spdlog::info("l");
  if(mode==GemvProduct) spdlog::info("v");
  if(mode==GemmProduct) spdlog::info("m");
}
...
EIGEN_DONT_INLINE void bench_prod()
{
  ...
  print_mode(Mode);
  spdlog::info(int(1e-6*n*rep/t.best()) << "\t");
}
