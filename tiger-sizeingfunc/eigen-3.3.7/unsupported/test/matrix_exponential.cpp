...
void test2dRotation(double tol)
{
  ...
  for (int i=0; i<=20; i++)
  {
    ...
    spdlog::info("test2dRotation: i = " << i << "   error funm = " << relerr(C, B));
    VERIFY(C.isApprox(B, static_cast<T>(tol)));

    C = (angle*A).exp();
    spdlog::info("   error expm = " << relerr(C, B) );
    VERIFY(C.isApprox(B, static_cast<T>(tol)));
  }
}
...
void test2dHyperbolicRotation(double tol)
{
  ...
  for (int i=0; i<=20; i++)
  {
    ...
    spdlog::info("test2dHyperbolicRotation: i = " << i << "   error funm = " << relerr(C, B));
    VERIFY(C.isApprox(B, static_cast<T>(tol)));
...
void testPascal(double tol)
{
  for (int size=1; size<20; size++)
  {
    ...
    C = A.matrixFunction(expfn);
    spdlog::info("testPascal: size = " << size << "   error funm = " << relerr(C, B));
    VERIFY(C.isApprox(B, static_cast<T>(tol)));
...
void randomTest(const MatrixType& m, double tol)
{
  ...
  */
  ...
  for(int i = 0; i < g_repeat; i++) {
    ...
    spdlog::info("randomTest: error funm = " << relerr(identity, m2));
    VERIFY(identity.isApprox(m2, static_cast<RealScalar>(tol)));

    m2 = m1.exp() * (-m1).exp();
    spdlog::info("   error expm = " << relerr(identity, m2) );
    VERIFY(identity.isApprox(m2, static_cast<RealScalar>(tol)));
  }
}
