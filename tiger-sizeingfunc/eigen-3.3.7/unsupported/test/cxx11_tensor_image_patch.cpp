          ...
          for (int d = 0; d < 2; ++d) {
            for (int b = 0; b < 7; ++b) {
              ...
              if (entire_image_patch(d, r, c, patchId, b) != expected) {
                spdlog::info("Mismatch detected at index i=" << i << " j=" << j << " r=" << r << " c=" << c << " d=" << d << " b=" << b);
              }
...
void test_patch_no_extra_dim()
{
  ...
  for (int i = 0; i < tensor.size(); ++i) {
    // ColMajor
    if (tensor.data()[i] != single_pixel_patch.data()[i]) {
      spdlog::info("Mismatch detected at index " << i << " : " << tensor.data()[i] << " vs " << single_pixel_patch.data()[i]);
    }
    VERIFY_IS_EQUAL(single_pixel_patch.data()[i], tensor.data()[i]);
    // RowMajor
    if (tensor_row_major.data()[i] != single_pixel_patch_row_major.data()[i]) {
        ...
        for (int c = 0; c < 5; ++c) {
          for (int d = 0; d < 2; ++d) {
            ...
            if (entire_image_patch(d, r, c, patchId) != expected) {
              spdlog::info("Mismatch detected at index i=" << i << " j=" << j << " r=" << r << " c=" << c << " d=" << d);
            }
