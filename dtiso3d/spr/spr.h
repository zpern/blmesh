...
#define MAX_SPR_POLY_SIZE 4096//default is 300
#define MAX_CONSTRAINT_SIZE 2048
...
{
	int i1 = 0, i2 = 0;						   /* 边所在的2个端点 */
	int j1 = 0, j2 = 0, j3 = 0;					   /* 必须消除的边或面，如果i3 = -1表示是一条边 */
	EDG_CONS_TYPE type = MUST_EXIST;				   /* 类型 */
	InstEntList *pListAllowed = nullptr;		   /* 允许相交的实体 */
	InstEntList *pListExcluded = nullptr;		   /* 不允许相交的实体 */
} EdgeConstraint;
...
public:
	SPRImpl();
	int init_spr();
	int small_poly_reconn(polyhedron *poly, divide *divi, double iniQ, Constraints *csts = NULL, int *smallPolySize = NULL, int maxIntNum = 0);
	 ...
	 * --------------------------------------------------------------------------------------*/
#ifdef _SPR_USING_STD_VECTOR
	int record_subproblems(double iniQ, struct polyhedron *poly, int f, int maxSize, divide_record *divR,
		std::vector<SubDivideProblem*>& vec_sub_prob,
		Constraints *csts, int maxIntNum);

	int record_subproblems(double iniQ, struct polyhedron *poly, divide_record *divR,
		std::vector<SubDivideProblem*>& vec_sub_prob,
		Constraints *csts, int maxIntNum);

	int optimal_twoStep_firstCall(double iniQ, polyhedron *simpPolyArray[], int simpPolyNum, divide *dive, divide_record *divR,
		Constraints *csts, int maxIntNum);

	int optimal_twoStep(double iniQ, std::vector<polyhedron *>& vec_simp_poly, divide *dive, divide_record *divR,
		Constraints *csts, int maxIntNum)
#else
	int record_subproblems(double iniQ, struct polyhedron *poly, int f, int maxSize, divide_record *divR,
	...
	int optimal_fourFace(double iniQ, polyhedron *poly, divide *dive, divide_record *divR, Constraints *csts, int maxIntNum);

	int optimal_twoStep_firstCall(double iniQ, polyhedron *simpPolyArray[], int simpPolyNum, divide *dive, divide_record *divR,
		Constraints *csts, int maxIntNum);

	int optimal_twoStep(double iniQ, polyhedron *simpPolyArray[], int simpPolyNum, divide *dive, divide_record *divR,
		Constraints *csts, int maxIntNum);
...
#endif
	int optimal_twoStep(double iniQ, polyhedron *poly, divide *dive, divide_record *divR,
		Constraints *csts, int maxIntNum);
	...
	int isNewFacetsValid_BruteForce(polyhedron *poly, int newCnt, int newIdx[],
		Constraints *csts);
 ...
 * -------------------------------------------------------------------*/
	...
	/* 计算相对值 */
	double calc_facet_quality_rel(polyhedron *poly, int fIdx, polyhedron *pld_poly, double mindst); //zhaodawei
	double calc_facet_quality_rel(polyhedron *poly, int fIdx, double mindst);
	...
	int calc_worst_facet(polyhedron *poly);
 ...
 * -------------------------------------------------------------------*/
	 ...
	 * 对中间结果进行记录
	 * -------------------------------------------------------------------*/
	int is_nonoverlap_tet(divide_record *oldRd, int key, int *pos);
	int create_div_record(divide_record *oldRd, int key, divide_record *newRd, int *found);
	void binarySearch(int * loc, int * found, int Item, int Array[], int right, int left);
	int is_tested_div_record(divide_record *cur_record);
	void add_div_record(divide_record *cur_record);
 ...
 * 对面片和节点进行排序
 * -------------------------------------------------------------------*/
	void sort_facets(polyhedron *poly);
	int get_sorted_facet(polyhedron *poly, int fIter);
	void sort_nodes(polyhedron *poly, int fDig);
	int get_sorted_node(polyhedron *poly, int nIter);
...
#ifdef _SPR_USING_STD_VECTOR
	int sort_subproblems(std::vector<SubDivideProblem*>& vec_sub_prob);
	...
	int istetra(int ELE[], struct polyhedron *P, double q, double *quality_ELE,
		...
		Constraints *csts = NULL, int maxIntNum = 0, int *intNum = NULL);
#else
	int sort_subproblems(SubDivideProblem *subProbArray[], int subProbNum);
	int delELE(int ELE[4], struct polyhedron *P,
		struct polyhedron *Q,
...
private:
	...
	double compEdgeNarrowDist(polyhedron *simp_poly, int a, int b, polyhedron *old_poly, double mindst);
	int calcInstPointNumber_LineCst(polyhedron *poly, int newCnt,
		Constraints *csts, int maxIntNum, int *intNum);
	bool comp_subproblems(const SubDivideProblem *prob1, const SubDivideProblem *prob2);

	bool comp_subproblemsV2(const void *arg1, const void *arg2);

	double compNarrowDist(polyhedron *simp_poly, polyhedron *old_poly);

	int islinecrossface(TRI face1, TRI face2, APOINT facep1[], APOINT facep2[], int m, int n, int res, struct polyhedron *P);

	int eval_one_subprob_LJF(SubDivideProblem *subProb);

	int eval_one_subprob(SubDivideProblem *subProb);

#ifdef _SPR_USING_STD_VECTOR
	int eval_subproblems(std::vector<SubDivideProblem*>& vec_sub_prob);
#else
	int eval_subproblems(SubDivideProblem *subProbArray[], int subProbNum);
#endif
public:
	EdgeConstraint edgeConsArray[MAX_CONSTRAINT_SIZE];
	/* spr_optimal_type = EDGE_REMOVAL时，初始壳包含边的数目 */
	int count = 0;		//四面体有效性的判断次数
	int real = 0;		//实际的递归次数
	int box_n = 0;		//用box消掉的面面相交的次数
	int face_n = 0;		//总共的面面相交的判断次数
	int line_n = 0;		//总共的线面相交的判断次数
	int box_l_n = 0;	//用box消掉的线面相交的次数
	int float_count = 0;	//总共的用浮点计算判断的面面相交的次数
	long orient3d_n = 0;		//计算体积的次数
	long Call_V = 0;	//试图查找次数
	long Call_V_R = 0;	//从已知数据中调用的次数
	long Call[10]{ 0 };


	double start, stop = 0;	//记录整体时间
	double duration = 0;

	double starttmp[10]{ 0 }, stoptmp[10]{ 0 };	//记录细节时间
	double dura[10]{ 0 };
	double quality_calc_time = 0.0;
	int m_QualityMeasureSPR = -1;	/* 优化参数 */

#define INT_MAX ((int)(~0U>>1))
	APOINT_ARRAY vertices;
	int num_vertices = 0;
	bool spr_opt_solution = true;	/* 是求最优解，还是求一个可行解即可：true 最优解；false: 可行解 */
	double spr_time_start = 0.0;    /* 记录操作开始的时间 */
	double spr_time_limit = 0.0;	/* 单次操作的时间限制(指单个多面体，一个问题可能包含多个多面体，time_limit = 0.0表示不限制) */
	int spr_optimal_type = MAX_MIN_CELL_QUALITY;
	int spr_max_int_node = 0;       /* spr_optimal_type == MIN_INT_NODE_NUMBER时有效 */
	//static int spr_int_node_num = 0; /* 临时变量，记录新面片和约束相交的次数 */
	bool spr_no_more_recur = false; /* 临时变量，用来记录是否需要提前终止递归：
									 * 情形1：超时 */
	bool spr_verbose_output = false;

	/* for test only */
	int spr_l2g[MAX_SPR_POLY_SIZE];
	Constraints spr_csts;

	int edgeConsArraySize = 0;
	int spr_shell_edge_nodeS = -1;
	int spr_shell_edge_nodeE = -1;

	/* spr_optimal_type = EDGE_REMOVAL时，初始壳包含边的数目 */
	int spr_shell_size = 0;
	int spr_allowed_face_num = 0;	/* 记录子问题允许包含的面片数最大值，临时变量 */
	int spr_subprob_face_num = 0;	/* 记录子问题实际包含的面片数，临时变量 */
	int spr_dependent_edges[MAX_CONSTRAINT_SIZE][3]; /* 第3个成员为类型。=0：没有顶点为壳边顶点；=1：有一个顶点为壳边端点； */
	int spr_dependent_edges_cnt = 0;

#ifdef _SPR_MAP_TETRA_VOLUME
	std::map<int, double> mapTetraVolume;
#endif /* _SPR_MAP_TETRA_VOLUME */

	std::set<divide_record, RuntimeCmp<divide_record> > setDivideRecord;	/* 记录节点在环内的编号 */

	//divide_poly variable
	IntStack dp_stkTreeSear;
	int dp_g2l[MAX_SPR_POLY_SIZE];

	//fill_inserted_tetra variable
	int fit_fac3_arr[MAX_SPR_POLY_SIZE]{ 0 }, fit_fac4_arr[MAX_SPR_POLY_SIZE]{ 0 },
		fit_cod3_arr[MAX_SPR_POLY_SIZE]{ 0 }, fit_cod4_arr[MAX_SPR_POLY_SIZE]{ 0 },
		fit_g2l[MAX_SPR_POLY_SIZE]{ 0 }, fit_nodeParents[MAX_SPR_POLY_SIZE]{ 0 };

	//sort_comp_edges variable
	int sce_compPolyID = 0, sce_oddPolyID = 0;

	//calc_worst_facet variables
	double cwf_norms[MAX_SPR_POLY_SIZE][4]; /* record the normal vector and its magnitude**2 */
	char cwf_faceNormalised[MAX_SPR_POLY_SIZE]{ 0 };
};


#ifdef _SPR_MAP_TETRA_VOLUMESPRlogTime
...
namespace SPR
{
	//extern int m_QualityMeasureSPR;	/* 优化参数 */
