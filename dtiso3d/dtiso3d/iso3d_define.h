 ...
 */
#define SUR_EDG_RATIO (1.5 * FAC_AMPLIFY_FACTOR)
...
typedef struct SurTri
{
	...
	INTEGER neig[3];	/* 邻居，为适应非二面流型，共享一条边的邻居形成一个环表 */
...
typedef struct SurEdg
{
	...
	INTEGER face;		  /* 指向第一个包含这条边的面片 */
	INTEGER	iHashNxt;     /* 哈希表指针，指向下一条边 */
...
typedef struct SubFacet
{
	INTEGER form[3];	/* 形成节点 forming points */
	INTEGER iNxt;		/* 辅助信息 help info，哈希表指针，指向下一张面 */
} SubFacet;
...
#define MAX_CLUSTER_SIZE 8 * 2048
/* 候选簇元大小最大值 max. size of candidate clusterels */
#define MAX_CLUS_CAND_SIZE 8 * 2048
 ...
 */
#define DES_EDG_RATIO ((REAL)(0.001))
#define DES_FAC_RATIO ((REAL)(0.001))
#define SUB_FAC_RATIO ((REAL)(0.003))
#define MIN_DES_EDG ((INTEGER)20)
#define MIN_DES_FAC ((INTEGER)20)
#define MIN_SUB_FAC ((INTEGER)60)
#define DEF_DES_EDG(x) ((INTEGER) (DES_EDG_RATIO * x))
#define DEF_DES_FAC(x) ((INTEGER) (DES_FAC_RATIO * x))
#define DEF_SUB_FAC(x) ((INTEGER) (SUB_FAC_RATIO * x))
...
/* x为表面面片总数 x represents the num. of faces */
#define INIT_SUB_FAC(x) (DEF_SUB_FAC(x) < MIN_SUB_FAC ? MIN_SUB_FAC : DEF_SUB_FAC(x))
 ...
 */
#define MAX_SKIRT_POLY_SIZE 2048 
#define MAX_ANGLE_INTERVAL_SIZE 128
...
//使用尺寸场进行尺寸控制
#define USE_SIZING_FUNC

#ifdef USE_SIZING_FUNC
//只使用尺寸场进行尺寸控制
//#define PURE_SIZING_FUNC_
#endif // USE_SIZING_FUNCTION

#define DO_ZONE_DIVIDING //进行子域划分
#define OUTPUT_ZONE_TAG //输出网格时输出区域信息
#define OUTPUT_CENTRIC_POINT_INFO //按高阶网格输出边中点信息
