...
public:
	...
	/*
	* read surface mesh from vtk file
	*/
	int readSurfVTK(const char* fname);
	...
	int writeVtkFile(const char* fname, bool outputZoneTag = false, bool outputCentricPoint = false);

	int writeVisualVtkFile(const char* fname);

	void prepareSurfTriOut(ZoneAttachInfo* zoneInfo, std::vector<int>& triElems, std::vector<std::vector<int>>& triFaces, std::vector<int>& triToDomain);
	/* -----------------------------------------------------------
	 * 检查表面是否自相交 
	 * ---------------------------------------------------------*/
	...
	int isBndEdge(INTEGER i1, INTEGER i2);

	//目前仅适用于bw插点环节
	bool isBndFace(INTEGER i1, INTEGER i2, INTEGER i3);
	void initBndFaceSet();
	void addBndFace(INTEGER i1, INTEGER i2, INTEGER i3);
...
//	void disableEleTstFlag(INTEGER iEle);
	...
	bool isMeshEdge_slow(INTEGER i1, INTEGER i2);
	...
	INTEGER add_box_edges();
	INTEGER abstEdges();
	void clearEdges();
	INTEGER getEdge(INTEGER i1, INTEGER i2, INTEGER *iLast);
	int isExteriorFacet(INTEGER i1, INTEGER i2, INTEGER i3);
	INTEGER addEdge(INTEGER i1, INTEGER i2, INTEGER iLast);
	int resetSurEdgHash(INTEGER maxSize);

	INTEGER getSubFacet(INTEGER i1, INTEGER i2, INTEGER i3, INTEGER *iLast);
	INTEGER addSubFacet(INTEGER i1, INTEGER i2, INTEGER i3, INTEGER iLast);
	...
	int classifyBndEdges_BruteForce(std::vector<int>& lostEdge);
	int classifyBndFaces(INTEGER *nLost, INTEGER *nSPs = NULL);
	//中点插点法
	int recvBndEdge_MidStnPnt();
	int splitEdge(INTEGER iEdg, int level);
...
//	}
	...
	int addSubFacetHash(INTEGER iFacet, INTEGER iDesFacet);
	int initSubFacetHash(INTEGER nodeSize);
	...
	/* sum the volume of interior elements */
	int computeInteriorVolume(REAL *vol);
	/*
	...
	*/
	...
	int writePL3(int *npt, double **gpt, int *nelm, int** elm);
	/*
	...
	*/
	 ...
	 * write .inp file
	 */
	int writeINP(const char *filename);
...
//	int checkInOut();
	...
	typedef std::function<double(std::array<double,3>) >  pfun;
	pfun GetSizingFuntion() {
		return sizeingfunction;
	}
	void SetSizingFuntion(pfun function) {
		sizeingfunction = function;
	}
#ifdef _TEST
	/*
...
protected:
	...
	int addInnerNode_Robust(INTEGER iNod, INTEGER iBaseE, bool instInner = true);
	...
	int calc_sphere_quality(Sphere sph, int nSph,
		std::map<int, int>& mapG2L, double *qual, INTEGER l2g_nod[], INTEGER &numv, bool bCalcQuality); // add by zjj 忘记与前两个函数的区别了...
	/* 将分解结果加入到当前四面体化中 */
...
public:
	...
	void meshopt(int smoothAttempts);
	...
	//小多面体重连（Small Polyhedron Reconnection: SPR）
	int SPRImproveQuality(int nMaxPass, REAL qualLimit = 0.5, int qualmeasure = QUALWARPEDMINSINE); //将质量值低于threshold的网格单元进行SPR操作
	int findSphereElem(INTEGER iElem, Sphere sph, int *nSph);	//获得与单元共点的球
	int findShellBadEdges(INTEGER iElem, Shell she, int *nShe);  //根据单元的两条差边获得相应的壳
	int FindLarge2Vals(float *adDihe, int &secondL); //找到二面角最大的两条边
	int findShellBdyEdge(INTEGER e, INTEGER iElem, Shell she, int *nShe); //找到环绕边界边的所有单元
	int findNxtElem(int edge[], int elemIdx, int *iNeig, bool clockwise = false);
	...
	int small_ball_reconn(polyhedron *poly, divide *divi, double minQ_forAll, double minQ_forBnd, int qualmeasure, int *nodeFlags, int *iAdjNode);
	int remove_PFTs();
	...
	//四面体网格质量辅助函数
	float tetqual(INTEGER a, INTEGER b, INTEGER c, INTEGER d, float* quals, bool &bAcute, QUALTYPE qtype = DIHEDRAL_ANGLE_SINE);
	...
	//#TODO 128改为MAX_SKIRT
	void cyclingShellQual(int a, int b, int faceNum, int* faceNodes, int (*matrix_k)[MAX_SKIRT_POLY_SIZE],
		int* ijCycling, int ijCyclingCnt, std::set<float>& cylingQuals, int &cyclingTetNum);
	void qualRecursive(int a, int b, int (*matrix_k)[MAX_SKIRT_POLY_SIZE], int i, int j, int faceNum, int *faceNodes,
	...
	void fillInShellElems(int a, int b, int skirtSize, int *skirtNodes, int (*matrix_k)[MAX_SKIRT_POLY_SIZE],
		int *ijCycled, int ijCycledCnt, int newElemsConns[], int &newElemsSize);
	void fillInShellElemsRecursive(int a, int b, int (*matrix_k)[MAX_SKIRT_POLY_SIZE], int i, int j, int skirtSize, int *skirtNodes,
		int *ijCycled, int ijCycledCnt, int newElemsConns[], int &newElemsSize);
	...
	double checkMinVolumn(int* pidx = nullptr);
...
public:
	...
	int outVolMsh(TigerVolmTet* pVolmTet);
	int outSurMsh(TigerMesh* pSurfTet);

	//为了线程安全，将全局变量置入类中
	INTEGER g_nInnTotalCaviElem = 0;
	INTEGER g_nInnInstPoint = 0;
	INTEGER g_nInnTryPoints;
	INTEGER g_nInnInitElems;

	int g_nSPRCallings = 0;
	int g_nSPRCallSucc = 0;
	int g_nSPRPolySize = 0;

	int g_nGlobalSmoothingCallings = 0;
	int g_nEdContSmoothingCallings = 0;
	int g_nEdSpltSmoothingCallings = 0;
	int g_nGlobalSmoothingCallSucc = 0;
	int g_nEdContSmoothingCallSucc = 0;
	int g_nEdSpltSmoothingCallSucc = 0;

	/* 考察成功edge contraction(EC)操作在点优化操作调用前、EC操作执行前后网格质量的比例 */
	double g_dECQualityRatio_MinRatio = 1.0e100;
	double g_dECQualityRatio_MaxRatio = -1.0e100;
	double g_dECQualityRatio_Divide[MAX_EC_QUALITY_RATIO_DIVIDE + 1];
	int	g_nECQualityRatio_SuccStatistics[MAX_EC_QUALITY_RATIO_DIVIDE];
	int	g_nECQualityRatio_TotalStatistics[MAX_EC_QUALITY_RATIO_DIVIDE];

	int g_nEdContCallings;
	int g_nEdContCallSucc;
	int g_nEdContCallNewP;

	double time_start[MAX_TIME_IDX];
	double time_final[MAX_TIME_IDX];
	double time_elaps[MAX_TIME_IDX];
	int sphere_size_eval = 0;
protected:
	...
	std::set<std::tuple<int,int,int>> surfTriGraph; //仅用于bw插点环节
protected:
	/* *********************************************************************************
	 * 实现健壮的点插入内核所需的辅助数据结构
	 * 已知条件: 
	...
	SubFacet *m_pSubFacets;
	INTEGER m_nSubFacets;
	INTEGER m_nAllocSubFacets;


	/* 为原始的边界边创建哈希表 */
	INTEGER *m_pSurEdgHash;
	INTEGER  m_nSurEdgeHash_MaxIdx;
	INTEGER *m_pSubFacetHash;
	INTEGER m_nSubFacetHash_MaxIdx;
	std::map<pair<INTEGER, INTEGER>, int> is_surface_edge;
	/* scale factor & scale center*/
	REAL m_scale;						/*scale factor*/
	REAL m_scale_recip;
	REAL m_boundary_size_ratio;          /*boundary size function ratio*/
...
protected:
	...
	bool m_bEnableNodeSmoothing;
	bool m_bEnableEdgeRemoval;
	bool m_bEnableFaceRemoval;
	bool m_bEnableEdgeContract;
	bool m_bEnableEdgeSplitting;
	bool m_bEnableSPRImprove;

	//Dynamic Programming Edge Removal 算法相关变量			
	INTEGER rs_faceNodes[MAX_SHELL_SIZE], rs_faceParents[2 * MAX_SHELL_SIZE];
	int rs_faceNodesDep[MAX_SKIRT_POLY_SIZE], rs_ijCycling[MAX_SKIRT_POLY_SIZE], rs_ijCycled[MAX_SKIRT_POLY_SIZE], rs_visited[MAX_SKIRT_POLY_SIZE];
	int rs_matrix_q[MAX_SKIRT_POLY_SIZE][MAX_SKIRT_POLY_SIZE], rs_matrix_k[MAX_SKIRT_POLY_SIZE][MAX_SKIRT_POLY_SIZE];
	int rs_matrix_f[MAX_SKIRT_POLY_SIZE][MAX_SKIRT_POLY_SIZE], rs_matrix_b[MAX_SKIRT_POLY_SIZE][MAX_SKIRT_POLY_SIZE];
	int rs_matrix_p[MAX_SKIRT_POLY_SIZE][MAX_SKIRT_POLY_SIZE]; /* 记录和边界边相交的次数  */
	int rs_skirtFaceSPs[MAX_SKIRT_POLY_SIZE];
	int rs_nSearchStatus[MAX_SKIRT_POLY_SIZE][MAX_SKIRT_POLY_SIZE], rs_nSearchLimit[MAX_SKIRT_POLY_SIZE];
	
	//shelltransform
	INTEGER st_faceNodes[MAX_SHELL_SIZE], st_faceParents[2 * MAX_SHELL_SIZE];
	int st_faceNodesDep[MAX_SKIRT_POLY_SIZE], st_ijCycling[MAX_SKIRT_POLY_SIZE], st_ijCycled[MAX_SKIRT_POLY_SIZE], st_bShellTrans[MAX_SKIRT_POLY_SIZE];
	float st_tetraQuala[MAX_SKIRT_POLY_SIZE], st_tetraQualing[MAX_SKIRT_POLY_SIZE * 2 - 4], st_tetraQualed[MAX_SKIRT_POLY_SIZE * 2 - 4];	//记录网格质量
	float st_matrix_q[MAX_SKIRT_POLY_SIZE][MAX_SKIRT_POLY_SIZE], st_matrix_b[MAX_SKIRT_POLY_SIZE][MAX_SKIRT_POLY_SIZE];
	int st_matrix_k[MAX_SKIRT_POLY_SIZE][MAX_SKIRT_POLY_SIZE];
	int st_matrix_f[MAX_SKIRT_POLY_SIZE][MAX_SKIRT_POLY_SIZE];
	int st_nSearchStatus[MAX_SKIRT_POLY_SIZE][MAX_SKIRT_POLY_SIZE], st_nSearchLimit[MAX_SKIRT_POLY_SIZE];

	//shelltransform_multi_removal
	int stm_ijCycling[MAX_SKIRT_POLY_SIZE], stm_ijCycled[MAX_SKIRT_POLY_SIZE], stm_bShellTrans[MAX_SKIRT_POLY_SIZE];
	float stm_tetraQuala[MAX_SKIRT_POLY_SIZE], stm_tetraQualing[MAX_SKIRT_POLY_SIZE * 2 - 4], stm_tetraQualed[MAX_SKIRT_POLY_SIZE * 2 - 4];	//记录网格质量
	INTEGER stm_oldElemsArray[2 * MAX_SKIRT_POLY_SIZE - 4], stm_newElemsArray[2 * MAX_SKIRT_POLY_SIZE - 4], stm_newElemsConns[8 * MAX_SKIRT_POLY_SIZE - 16];
	float stm_matrix_q[MAX_SKIRT_POLY_SIZE][MAX_SKIRT_POLY_SIZE], stm_matrix_b[MAX_SKIRT_POLY_SIZE][MAX_SKIRT_POLY_SIZE];
	int stm_matrix_k[MAX_SKIRT_POLY_SIZE][MAX_SKIRT_POLY_SIZE];
	int stm_nSearchStatus[MAX_SKIRT_POLY_SIZE][MAX_SKIRT_POLY_SIZE], stm_nSearchLimit[MAX_SKIRT_POLY_SIZE];

	REAL minCri = 1.0e10;

	//Global  InstEnt
	InstEntList listDependentEnts_BE_V2;
	InstEntList listDependentEnts_BF_V2;
	InstEntList listTestedEnts_findAllInstEntitiesOfBF;

	//for smooth
	REAL globalMinQual = 1.0e100; /* 全局最小值  */

	LocSmoother* locsmt;

	EarRemover* earrmov;

	//MidPoint RecvBndEdge 相关变量
	const int MaxMidSPIteration = 15;
	bool addMPConflict;
	EdgeGrapher<bool> conflictEdg;
	std::vector<int> iEdgToRecv;
	EdgeGrapher<bool> recoveredEdg;
	const double MaxSplitFactor = 20; //当比例大于改比例时，消灭相邻边界边
	std::map<int, int> parentFaces; //记录中点插点法分解的表面的母表面
...
public:
	...
	inline bool hasSizingFunc();
	inline void setSizingFunc(sizingfunc_t sf);
	inline double sizingFunc(const double& x, const double& y, const double& z);

private:
	sizingfunc_t sizingfunc;
#endif

#ifdef DO_ZONE_DIVIDING
public:
	//目前由于不知道删除block时要维护哪些变量，可能会对后面的流程产生影响，因此该流程先放到最后做
	void solveZone();
	void setZoneInfo(std::shared_ptr < ZoneAttachInfo> info);
	int outVolMshZoneInfo(TigerMesh* pVolmTet);
	std::vector<std::vector<int>> getDomain2Blocks();
	std::vector<int> getElem2Domains();
	std::shared_ptr<ZoneAttachInfo> zoneInfo;

private:
	std::vector<std::vector<int>> domain2Blocks;
	std::vector<int> elemToDomains;
	std::vector<int> pthash_delblock; //delete block 之后的对应点哈希
...
/* 尺寸场相关函数 */
inline bool DTIso3D::hasSizingFunc() {
	return sizingfunc;
}
inline void DTIso3D::setSizingFunc(sizingfunc_t sf) { 
	sizingfunc = sf; 
}
inline double DTIso3D::sizingFunc(const double& x, const double& y, const double& z) {
	REAL pt[DIM]{ x,y,z };
	nToW(pt,&pt);
	REAL ret;
	wToN(sizingfunc(pt[0], pt[1], pt[2]), &ret);
	return ret;
}
